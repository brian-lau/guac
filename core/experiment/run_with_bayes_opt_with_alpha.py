        ...
        'like-dislike':
            hp.choice('like-dislike', [
                ...
                },
                {
                    ...
                    'ld_min_doc_threshold': hp.choice('ld_min_doc_threshold', [1,2,3,4,5,6])
                }
            ] ),
        'dem-rep':
            hp.choice('dem-rep', [
                ...
                },
                {
                    ...
                    'dr_min_doc_threshold': hp.choice('dr_min_doc_threshold', [1,2,3,4,5,6])
                }
            ] ),
        'POS_tags':
            hp.choice('POS_tags', [
...
def call_experiment(args):
    ...
    if reuse:
        result = experiment.run_group_experiment(name, datasets, 0, feature_list, model_type=model, reuse=True,
                                                 best_alphas=alphas, **kwargs)
    else:
        result = experiment.run_group_experiment(name, datasets, 0, feature_list, model_type=model,
                                                 best_alphas=alphas, **kwargs)
    print result
