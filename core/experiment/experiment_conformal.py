...
def get_item_dicts(datasets, test_fold, dev_subfold, calibration=None):
    train_dict = {}
    valid_dict = {}
    test_dict = {}
    for f in datasets:
        # add the training items for this dataset to the list
        train_dict[f] = ds.get_train_documents(f, test_fold, dev_subfold, calibration)
        valid_dict[f] = ds.get_dev_documents(f, test_fold, dev_subfold, calibration)
